...
function DynamicForm({
  ...
  return (
    <ScrollArea
      ...
      viewportClassName="mastra-actions-form-scroll-area"
    >
      <div className="flex flex-col h-full">
        ...
        <section className="flex flex-col gap-5 pt-6">
          <div className="flex flex-col gap-3 px-6">
            <Label
              className="capitalize flex gap-0.5"
              htmlFor="mastraConnectionId"
              aria-required={true}
            >
...
function InnerDynamicForm<T extends ZodSchema>({
  ...
  const discriminatedUnionSchemaDiscriminator = (block?.zodSchema as any)?._def
    ?.discriminator;
  ...
  const {
    ...
    resolver:
      blockSchemaTypeName === "ZodDiscriminatedUnion"
        ? customZodResolver(
            block?.schema as any,
            discriminatedUnionSchemaDiscriminator,
          )
  ...
  const schema =
    blockSchemaTypeName === "ZodDiscriminatedUnion"
      ? discriminatedUnionSchemaOptions?.find(
          (option: any) =>
            option?.shape?.[discriminatedUnionSchemaDiscriminator]?._def
  ...
  return (
    <>
      <form
        ...
        className="flex flex-col gap-8 p-6 pb-0 h-full"
      >
        {renderDynamicForm({
          schema,
          block,
...
function resolveSchemaComponent({
  ...
  if (schema instanceof z.ZodUnion) {
    if (schema.options.some((s: z.ZodType) => s instanceof z.ZodNull)) {
      const nonNullSchema = schema.options.find(
        (s: z.ZodType) => !(s instanceof z.ZodNull),
      );
  ...
  const fieldFromDescription = (schema as ZodSchema)?._def?.description?.split(
    "::",
  )[1];
