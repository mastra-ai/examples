...
export const saveAgent = async ({
  ...
  const agentsDirPath = await getAgentsDirPath();
  const agentWriter = new AgentWriterService(agentsDirPath);
  await agentWriter.writeAgent(
    path.join(agentsDirPath, `${agentId}.json`),
    data,
  );
};
...
export const getAgentsDirPath = async () => {
  ...
  return path.join(
    MASTRA_APP_DIR,
    framework?.config?.agents?.agentDirPath || "",
  );
};

export const saveApiKeyToEnvAction = async ({
  modelProvider,
  apiKey,
}: {
...
export const getApiKeyFromEnvAction = async (
  modelProvider: string,
): Promise<string | null> => {
  const rootPath = process.env.MASTRA_APP_DIR || process.cwd();
  const envPath = path.join(rootPath, ".env.development");
  const envReader = new FileEnvService(envPath);
  const apiKey = await envReader.getEnvValue(
